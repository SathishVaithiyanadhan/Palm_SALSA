    ...
    def add_variables(self):
        ...
        for tiff_file in glob.glob(os.path.join(self.tiff_dir, "emission_*_temporal.tif")):
            ...
            for band_idx, band_name in enumerate(band_names, 1):
                ...
                # Add to the appropriate emission category based on species
                if target_species in ["OC", "BC", "NH3"]:  # Traffic exhaust
                    traffic_by_hour[hour, :, :] += converted_arr
                elif target_species == "DU":  # Road dust
                    road_dust_by_hour[hour, :, :] += converted_arr
                elif target_species in ["OC", "BC"]:  # Wood combustion
...
if __name__ == "__main__":
    ...
    driver = SalsaDriver(static_file, tiff_dir, "/home/vaithisa/Palm_SALSA/Augsburg_3_salsa.nc", active_categories)
...
class SalsaDriver:
    ...
    def add_variables(self):
        ...
        for i, name in enumerate(emission_category_name_list):
            chars = list(name.ljust(self.nmax_string_length))
            nc_emission_category_name[i, :] = np.array(list(chars), dtype="S1")
        nc_emission_category_name.plugins_name = "emission category name"
...
if __name__ == "__main__":
    ...
    active_categories = [
        'F_RoadTransport',
        'C_OtherStationaryComb',  # Added for wood combustion
    ]
